...
- [ ] discover the eigenshapes from the segments
  ...
  - [ ] eliminate shapes that don't _enter_/_cross_ the boundary
...
- AH ok so for a bisector, we still have a bisector tool, but and the circle ticks for it are _decorations_. So not actually guides. but you can like move them around, recreationally.
...
- [ ] hmm one thing that a compass can do, is _transfer_ distances.
...
but it's behind a **dev** or whatever
...
- [x] for palettes, it would be nice to be able to paste in a 4- or 5-color one, and have it produce "1 shade lighter / darker", right? - or even better(?), just have a fill option that is darken/lighten? idk
