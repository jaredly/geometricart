...
export const PendingPathControls = ({
    ...
    onComplete: (isClip: boolean, origin: Coord, segments: Array<Segment>) => void;
}) => {
    const state = editorState.pending;
    if (state?.type !== 'path') {
        return null;
    }
    const setState = (pp: (path: EditorState['pending']) => EditorState['pending']) =>
            ...
            {isComplete(state) ? (
                <>
                    <button
                        ...
                        onClick={() => {
                            onComplete(
                                state.isClip,
                                state.origin.coord,
                                simplifyPath(ensureClockwise(state.parts.map((p) => p.segment))),
                            );
                        }}
                    >
                        finish ✅
                    ...
                    <button
                        css={{fontSize: 40, flex: 1}}
                        onClick={() => {
                            setState(backUp(state.origin, guidePrimitives, allIntersections));
                        }}
                    >
                        back ❌
                    </button>
                ...
                <>
                    ...
                    >
                        ...
                        <button
                            css={{fontSize: 40, flex: 1}}
                            onClick={() => {
                                if (state?.type === 'path' && isComplete(state)) {
                                    return onComplete(
                                        state.isClip,
                                        state.origin.coord,
                                        simplifyPath(
                                            ensureClockwise(state.parts.map((p) => p.segment)),
                                        ),
                                    );
                                }
                                setState(goForward(guidePrimitives, allIntersections));
                            }}
                        >
                            ✅
                        </button>
