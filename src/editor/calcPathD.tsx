...
export const calcSegmentsD = (
    ...
    let d = `M ${(origin.x * zoom).toFixed(prec)} ${(origin.y * zoom).toFixed(prec)}`;
    if (segments.length === 1 && segments[0].type === 'Arc') {
        ...
        return calcSegmentsD([{...arc, to: opposite}, arc], origin, open, zoom, prec);
        // this can only happen if we're a pure cicle
    }
    segments.forEach((seg, i) => {
        if (seg.type === 'Line') {
            d += ` L ${(seg.to.x * zoom).toFixed(prec)} ${(seg.to.y * zoom).toFixed(prec)}`;
        } else if (seg.type === 'Quad') {
            d += ` Q ${(seg.control.x * zoom).toFixed(prec)} ${(seg.control.y * zoom).toFixed(
                prec,
            )} ${(seg.to.x * zoom).toFixed(prec)} ${(seg.to.y * zoom).toFixed(prec)}`;
        } else {
