...
const initialBox = (tr: Coord) => [
    [[scaleMatrix(-1, 1)], [scaleMatrix(-1, 1), translationMatrix({x: tr.x * 2, y: 0})]],
...
export function initialTransform(shape: TilingShape, tr: Coord, tpts: Coord[]): Matrix[][][] {
    if (tpts.length === 4) {
        ...
        // Triangle -> Rectangle
        return [[[rotationMatrix(Math.PI), translationMatrix(tr)]], ...initialBox(tr)];
    } else if (closeEnough(tr.y, -1 / Math.sqrt(3))) {
        // HEX
        return [
            [[scaleMatrix(1, -1)]], //
            [
                ...[1, 2, 3, 4, 5].map((n) => [rotationMatrix(((Math.PI * 2) / 6) * n)]),
                [scaleMatrix(-1, 1), translationMatrix({x: tr.x * 2, y: 0})],
                [
                    rotationMatrix((Math.PI * 2) / 3),
                    translationMatrix({x: tr.x, y: -tr.x * Math.sqrt(3)}),
                ],
                [
                    rotationMatrix((Math.PI * 2) / 6),
                    translationMatrix({x: tr.x, y: -tr.x * Math.sqrt(3)}),
                ],
