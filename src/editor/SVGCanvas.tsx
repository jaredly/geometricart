...
export function SVGCanvas({
    ...
    setPendingMirror: (mirror: PendingMirror | ((mirror: PendingMirror | null) => PendingMirror | null) | null) => void;
}) {
    ...
    const finishDrag = React.useCallback(finishDragFn(setEditorState, currentDrag, currentState, multiRef, dispatch), []);

    const dragSelectHandlers = useDragSelect(editorState.dragSelectPos, width, height, view, setEditorState, finishDrag);
    const mouseDragHandlers = useMouseDrag(editorState.dragPos, setEditorState, width, height, view);

    const mouseHandlers = editorState.selectMode ? dragSelectHandlers : mouseDragHandlers;
    ...
    const dragged = editorState.dragSelectPos
        ? findSelection(currentState.current.paths, currentState.current.pathGroups, rectForCorners(editorState.pos, editorState.dragSelectPos))
        : null;

    const backgroundColor = view.background != null ? paletteColor(state.palette, view.background) : null;

    // Ok, so what I want is:
    // while dragging, freeze the bounds.
    const bounds = React.useMemo(() => calculateBounds(width, height, view), [width, height, editorState.dragPos ? null : view]);
        ...
        >
            <defs>
                {state.palette.map((color, i) =>
                    color.startsWith('http') && imageCache[color] ? (
                        ...
                        >
                            <image width={width} height={height} preserveAspectRatio="xMidYMid slice" href={imageCache[color] as string} />
                        </pattern>
                    ) : null,
                )}
            </defs>
            <g transform={`translate(${x} ${y})`}>
                {backgroundColor != null && backgroundColor !== 'transparent' ? (
                    <rect width={width} height={height} x={-x} y={-y} stroke="none" fill={backgroundColor} />
                ) : null}
                {Object.keys(state.overlays)
                    .filter(
                        (id) =>
                            maybeReverse(!state.overlays[id].hide, hover?.type === 'element' && hover?.kind === 'Overlay' && hover?.id === id) &&
                ...
                {pathsToShow.map((path, i) => (
                    <RenderPath
                        ...
                        onClick={
                            ...
                            editorState.pending !== null || uiState.pendingDuplication || uiState.pendingMirror ? undefined : clickPath
                        }
                    />
                ))}
                ...
                {state.selection && !styleHover
                    ...
                    : null}
                {dragged
                    ? dragged.map((id) => showHover(id, {kind: 'Path', id, type: 'element'}, state, mirrorTransforms, view.zoom, bounds, false))
                    : null}
                {hover ? <>{showHover(`hover`, hover, state, mirrorTransforms, view.zoom, bounds, false)}</> : null}
                {editorState.dragSelectPos ? (
                    <rect
                        ...
                        width={Math.abs(editorState.dragSelectPos.x - editorState.pos.x) * view.zoom}
                        height={Math.abs(editorState.dragSelectPos.y - editorState.pos.y) * view.zoom}
...
export function usePathsToShow(state: State) {
    ...
    let {res: pathsToShow, clip} = React.useMemo(() => {
        ...
        return {res, clip};
    }, [state.paths, state.pathGroups, state.clips, state.view.hideDuplicatePaths, state.view.laserCutMode, selectedIds]);
...
export function finishDragFn(
    ...
    return (shiftKey: boolean) => {
        ...
        const selected = findSelection(currentState.current.paths, currentState.current.pathGroups, rect);
        if ((shiftKey || multiRef.current) && currentState.current.selection?.type === 'Path') {
            selected.push(...currentState.current.selection.ids.filter((id) => !selected.includes(id)));
        }
