		...
		>
			...
			<g transform={`translate(${x} ${y})`}>
				...
				{editorState.dragSelectPos ? (
					<rect
						x={
							Math.min(editorState.dragSelectPos.x, editorState.pos.x) *
							view.zoom
						}
						y={
							Math.min(editorState.dragSelectPos.y, editorState.pos.y) *
							view.zoom
						}
						width={
							Math.abs(editorState.dragSelectPos.x - editorState.pos.x) *
							view.zoom
						}
						height={
							Math.abs(editorState.dragSelectPos.y - editorState.pos.y) *
							view.zoom
						}
	...
	React.useEffect(() => {
		state.palette.forEach((color) => {
			if (color.startsWith("http")) {
				...
				fetch(
					`https://get-page.jaredly.workers.dev/?url=${encodeURIComponent(color)}`,
				)
					.then((data) => data.blob())
					.then((blob) => {
