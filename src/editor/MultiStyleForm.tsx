							...
							<div css={{ display: "flex", flexWrap: "wrap" }}>
								...
								<div key={`lighten-${i}`}>
									<LightDark
										...
										onChange={(lighten) => {
											onChange(
												updateFill(styles, i, lighten ?? undefined, "lighten"),
											);
										}}
									/>
					...
					onChange(
						styles.map((style) => {
							...
							for (let fill of style.fills) {
								if (
									fill &&
									(at == null || (at.inset ?? 0) < (fill.inset ?? 0))
								) {
									at = fill;
								}
							}
							...
							<summary style={{ cursor: "pointer" }}>
								...
								{line.width.length === 1 && line.width != null && (
									<span>
										<BlurInt
											value={
												Math.round(pxToMM(line.width[0]! / 100, ppi) * 100) /
												100
											}
											onChange={(v) =>
												v != null
							...
							>
								...
								<div key={`opacity-${i}`}>
									...
									<MultiNumber
										...
										onChange={(opacity) => {
											onChange(
												updateLine(styles, i, opacity ?? undefined, "opacity"),
											);
										}}
									/>
								</div>
				...
				onClick={() => {
					...
					onChange(
						styles.map((style, i) => {
							const color =
								// @ts-ignore
								style.fills.findLast((f: Fill | null) => f?.color != null)
									?.color ??
								// @ts-ignore
								...
								style={{
									...
									boxShadow:
										...
										(value === 0 && lighten.includes(null))
											? `0 3px 0 ${
													allSame === value || (allSame == null && value === 0)
														? "white"
														: "orange"
												}`
											: "none",
