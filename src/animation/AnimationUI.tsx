...
export const AnimationEditor = ({
    ...
    let h = bounds ? makeEven((bounds.y2 - bounds.y1) * state.view.zoom + crop * 2) : originalSize;
    let w = bounds ? makeEven((bounds.x2 - bounds.x1) * state.view.zoom + crop * 2) : originalSize;
    ...
    return (
        <div style={{paddingBottom: 50}}>
            ...
            <TickTock t={animationPosition} increment={increment} set={setAnimationPosition} />
            <div
                style={{
...
export const Editable = ({text, onChange}: {text: string; onChange: (t: string) => void}) => {
    const [editing, setEditing] = useState(null as null | string);
    if (editing != null) {
        return (
            <>
                <input value={editing} onChange={(evt) => setEditing(evt.target.value)} />
                <button
                    onClick={() => {
                        setEditing(null);
                        onChange(editing);
...
export const AddVbl = ({onAdd}: {onAdd: (name: string, v: Animations['lerps']['']) => void}) => {
    const [key, setKey] = React.useState('vbl');
    const [low, setLow] = React.useState(0);
    const [high, setHigh] = React.useState(1);
    const [kind, setKind] = React.useState('float' as 'float' | 'float-fn' | 'pos-fn');
    return (
        <div
            style={{
        ...
        >
            ...
            <span style={{marginRight: 8}}>
                Kind:{' '}
                <button onClick={() => setKind('float')} disabled={kind === 'float'}>
                    Lines
                </button>
                <button onClick={() => setKind('float-fn')} disabled={kind === 'float-fn'}>
                    Float
                </button>
                <button onClick={() => setKind('pos-fn')} disabled={kind === 'pos-fn'}>
                    Pos
                </button>
            </span>
            {kind === 'float' ? (
