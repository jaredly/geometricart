...
export function Lerps({dispatch, state}: {dispatch: (action: Action) => unknown; state: State}) {
    return (
        <div style={{flex: 1, overflow: 'auto'}}>
            ...
            {Object.keys(state.animations.lerps).map((key) => {
                ...
                if (vbl.type !== 'float') {
                    return <ScriptLerp key={key} id={key} vbl={vbl} dispatch={dispatch} />;
                }
...
function FloatLerp({
    ...
    if (!current) {
        return (
            ...
            >
                {key}
                <PointsViewer onClick={() => setCurrentInner(vbl)} points={vbl.points} />
                <button
                    onClick={() => {
                        dispatch({type: 'timeline:update', key, vbl: null});
                    }}
            ...
            <PointsEditor
                ...
                setCurrentInner={(points) =>
                    typeof points === 'function'
                        ? setCurrentInner((v) => (v ? {...v, points: points(v.points)} : v))
