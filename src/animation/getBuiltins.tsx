...
export function getBuiltins(): {[key: string]: Function | number} {
    return {
        ...
        limit: (t: number, min: number, max: number) => Math.min(Math.max(t, min), max),
        segmentsBounds,
        segmentsCenter,
        transformSegment,
        applyMatrices,
...
const followPath = (path: Array<Coord> | Path, percent: number) => {
    ...
    if (!Array.isArray(path)) {
        points = rasterSegPoints(pathToPoints(path.segments, path.open ? path.origin : null));
...
const modFills = (path: Path, mod: (fill: Fill, i: number) => Fill | null): Path => {
    return {
        ...path,
        style: {
            ...path.style,
...
const modInsets = (path: Path, mod: (inset: number | undefined) => number | undefined): Path => {
    return {
        ...path,
        style: {
