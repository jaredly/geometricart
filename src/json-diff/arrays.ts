export type ArrayDelete<T extends {id: string}> = {
    type: 'delete';
    id: string;
    from: number;
    value: T;
};

export type ArrayInsert<T extends {id: string}> = {
    type: 'insert';
    id: string;
    at: number;
    value: T;
};

export type ArrayMove<T extends {id: string}> = {
    type: 'move';
    id: string;
    from: number;
    to: number;
    value: T;
};

export type ArrayEdit<T extends {id: string}> = ArrayDelete<T> | ArrayInsert<T> | ArrayMove<T>;

/**
 * Diff two arrays of items with stable, unique ids.
 *
 * Produces a sequence of deletes, inserts, and moves that turn `first` into `second`.
 * Indexes are relative to the in-progress working array if ops are applied in order.
 */
export function diffById<T extends {id: string}>(first: T[], second: T[]): ArrayEdit<T>[] {
    const neededCount = new Map<string, number>();
    second.forEach((item) => neededCount.set(item.id, (neededCount.get(item.id) ?? 0) + 1));
    const working = [...first];
    const ops: ArrayEdit<T>[] = [];

    // Remove extra occurrences while keeping the earliest ones.
    const seenCount = new Map<string, number>();
    const deleteIndices: number[] = [];
    for (let i = 0; i < working.length; i++) {
        const item = working[i];
        const needed = neededCount.get(item.id) ?? 0;
        const seen = (seenCount.get(item.id) ?? 0) + 1;
        seenCount.set(item.id, seen);
        if (seen > needed) deleteIndices.push(i);
    }
    for (let i = deleteIndices.length - 1; i >= 0; i--) {
        const idx = deleteIndices[i];
        const item = working[idx];
        working.splice(idx, 1);
        ops.push({type: 'delete', id: item.id, from: idx, value: item});
    }

    // Walk target order, moving or inserting as needed.
    for (let target = 0; target < second.length; target++) {
        const desired = second[target];
        const current = working[target];

        if (current && current.id === desired.id) continue;

        const found = working.findIndex((item, idx) => idx > target && item.id === desired.id);

        if (found !== -1) {
            const [item] = working.splice(found, 1);
            working.splice(target, 0, item);
            ops.push({type: 'move', id: item.id, from: found, to: target, value: item});
        } else {
            working.splice(target, 0, desired);
            ops.push({type: 'insert', id: desired.id, at: target, value: desired});
        }
    }

    return ops;
}
