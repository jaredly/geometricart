...
export const canvasRender = (
	...
	const { paths: animatedPaths, view } = state.animations.timelines.some(
		(t) => t.items.length,
	)
	...
	const rough =
		view.sketchiness && view.sketchiness > 0
	...
	if (view.background != null) {
		...
		const color =
			typeof view.background === "number"
				? palette[view.background]
				: view.background;
	...
	const spaths = sortedVisibleInsetPaths(
		animatedPaths,
		state.pathGroups,
		rand,
		clip,
	...
	spaths.forEach((path) => {
		...
		style.lines.forEach((line, i) => {
			...
			ctx.lineWidth =
				line.width === 0 ? (extraLarge ? 7 : 2) : (line.width / 100) * zoom;
			ctx.lineJoin = "bevel";
			// ctx.lineCap = 'square';
			ctx.lineCap = "round";
