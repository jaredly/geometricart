...
export const splitSegmentsByIntersections = (
    ...
    segments.forEach((segment, i) => {
        let prev = i === 0 ? segments[segments.length - 1].to : segments[i - 1].to;
        let intersection = 0;
        // if (segment.type === 'Arc') {
        //     console.log(segment, sorted[i], prev, segment.to);
        // }
        let hitStart = null as null | number;
        while (!coordsEqual(prev, segment.to, HIGH_PRECISION) && intersection <= sorted[i].length) {
            const key = coordKey(prev, HIGH_PRECISION);
            const to =
                intersection === sorted[i].length ? segment.to : sorted[i][intersection].coord;
            let hitEnd = intersection === sorted[i].length ? null : sorted[i][intersection].idx;
            if (!coordsEqual(prev, to, HIGH_PRECISION)) {
                ...
                if (segment.type === 'Arc') {
                    const initialAngle = segmentAngle(prev, {...segment, to}, true, true);
                    const finalAngle = segmentAngle(prev, {...segment, to}, false, true);
                    result.push({
                        prev,
                        segment: {...segment, to},
                        sourceIdx: i,
...
export function addPrevsToSegments(segments: Segment[], origin?: Coord): SegmentWithPrev[] {
    return segments.map((s, i) => ({
        prev: i === 0 ? (origin ?? segments[segments.length - 1].to) : segments[i - 1].to,
        segment: s,
        shape: -1,
    }));
}
...
export function calculateSortedHitsForSegments(
    ...
    const primitives = segments.map((s) => segmentToPrimitive(s.prev, s.segment));
            ...
            these.forEach((coord) => {
                if (!allowEndpoints) {
                    const iend =
                        ...
                        coordsEqual(coord, segments[i].segment.to, HIGH_PRECISION);
                    const jend =
                        coordsEqual(coord, prevj, HIGH_PRECISION) ||
                        coordsEqual(coord, segments[j].segment.to, HIGH_PRECISION);
