...
export const pointLine = (coord: Coord, line: SlopeIntercept) => {
	...
	return (
		closeEnough(y, coord.y) && (!line.limit || withinLimit(line.limit, coord.x))
	);
};
...
export function lineCircle(
	...
	if (d >= 0) {
		...
		if (d == 0) {
			if (
				limit &&
				!(limit[0] <= intersections[0].x && intersections[0].x <= limit[1])
			) {
				return [];
			}
			if (
				climit &&
				!angleIsBetween(angleTo({ x: cx, y: cy }, intersections[0]), climit)
			) {
				return [];
			}
			// only 1 intersection
